// https://stackoverflow.com/questions/38364065/is-it-possible-to-change-a-proxys-target
...
window.addEventListener('load', function() {
  console.log("Adding the googleapis script.");
  var tf = document.createElement('script');
  tf.type = 'text/javascript';
  tf.async = true;
  tf.src = '//maps.googleapis.com/maps/api/js?key=AIzaSyD2mCsnJ4TPH1ZfUK4iRhRF2MP4jF4SRRE&libraries=places';
  var s = document.getElementsByTagName('script')[0];
  s.parentNode.insertBefore(tf, s);
  console.log("Added the googleapis script.");
});
...
angular.module('google.places', [])
	 ...
	 */
	.factory('googlePlacesApi', ['$window', '$injector', function ($window, $injector) {
		const { proxy, setTarget } = mutableProxyFactory();
		
		var $rootScope = $injector.get('$rootScope');
		
		$rootScope.window = $window
		var checkGoogleInitialized = function() {
			if (typeof google !== 'undefined' && 'maps' in google) {
				  setTarget($window.google);
				  $rootScope.googleInitialized = true;
				}
		};
		$rootScope.$watch('window.google', checkGoogleInitialized);
		$rootScope.$watch('window.google.maps', checkGoogleInitialized);
        ...
        function ($parse, $compile, $timeout, $document, google) {
            ...
            return {
                ...
                link: function ($scope, element, attrs, controller) {
                    ...
                    if($scope.$root.googleInitialized) {
                    	autocompleteService = new google.maps.places.AutocompleteService();
                        placesService = new google.maps.places.PlacesService(element[0]);
                    } else {
                    	$scope.$watch('$root.window.google.maps', function() {
                    		if($scope.$root.googleInitialized) {
                    			autocompleteService = new google.maps.places.AutocompleteService();
                    			placesService = new google.maps.places.PlacesService(element[0]);
                    		}
